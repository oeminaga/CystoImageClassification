...
class Button(object):
    ...
    def Draw(self):
        ...
        self.vertices=ConvertPixelVerticiesToWorldCoordinate(self.vertices, w_height, w_width)
        self.edges = (
            (0,1),
            (1,2),
            (2,3),
    ...
    def OnClick(self, mous_coordination):
        a=Rectangle(self.position[0], self.position[0], self.position[0]+self.size[0], self.position[1]+self.size[1])
        b=Rectangle(mous_coordination[0], mous_coordination[1], mous_coordination[0]+10, mous_coordination[1]+10)
        x=area(a,b)
        if x>0:
...
class WindowVisualization:
    ...
    def GetScreenShot(self):
        ...
        data=pygame.image.tostring(screen_surf, 'RGBA')
    ...
    def Run(self):
        ...
        while True:
            ...
            gluPerspective(45, (self.width / self.height), 0.1, 10.0)#(width / height)
            glMatrixMode(GL_MODELVIEW)
            glLoadIdentity()
